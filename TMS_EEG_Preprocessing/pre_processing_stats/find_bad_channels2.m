function [bad_channels, lowfrontalrejs] = find_bad_channels2(EEG, threshold, frontal_threshold_shift, peak_thresh, peak_thresh2)
    data = EEG.data;
    ...
    %size(z_scores)
    [bad_channels, lowfrontalrejs] = search_bad_channels(z_scores, threshold, channel_locations, frontal_threshold_shift,[]);
    if ~isempty(bad_channels)
        ...
        while true
            ...
            [bad_channels_new, lowfrontalrej2] = search_bad_channels(new_z_scores, threshold, channel_locations, frontal_threshold_shift, bad_channels);
            lowfrontalrejs = union(lowfrontalrejs,lowfrontalrej2);
...
function [searched_bad_channels, lowfrontalrej] = search_bad_channels(z_scores, threshold, channel_locations, frontal_threshold_shift, exclude)
    ...
    for i=1:n_channels
        if ~ismember(i,exclude)
            ...
            if z_scores(z_score_channel) < -threshold && threshold_now ~= threshold && abs(z_scores(z_score_channel)) < threshold_now
                disp("prolly bad rej of channel")
                lowfrontalrej(end+1) = i;
            end
...
function threshold_now = get_adjusted_treshold(channel_locations, i, threshold, frontal_threshold_shift)
    ...
    elseif strcmp(chan_name(1),'f')
        if strcmp(chan_name(2),'f') || ~isnan(str2double(chan_name(2))) || strcmp(chan_name(2),'z')
