...
bad_trials_neg_z = [];
bad_trials_neg_z_post = [];
bad_chs_adjusted_neg_z = [];
frontalrejs = [];
for site=sites
    ...
    for index = 1:length(subject_names)
        ...
        [bad_channels_pre, frontalrejs1] = find_bad_channels2(EEG_pre_filtered, rejchan_threshold_pre, frontal_threshold_shift, pre_peak_threshes_large, pre_peak_threshes_small);
        frontalrejs(end+1) = length(frontalrejs1);
        disp("Searching for bad channels from post-stim...")
        [bad_channels_post, frontalrejs2] = find_bad_channels2(EEG_post_true, rejchan_threshold_post, [0 0], [], []);
        ...
        [bad_trials_pre, n_neg] = find_bad_trials2(EEG_pre, rejtrial_threshold_global_pre, rejtrial_thresholds_local_pre, nums_over_rejtrial_thresholds_local_pre);
        bad_trials_neg_z(end+1) = n_neg;
        ...
        [bad_trials_post, n_neg_post] = find_bad_trials2(EEG_post_in_post_range, rejtrial_threshold_global_post, rejtrial_thresholds_local_post, nums_over_rejtrial_thresholds_local_post);
        bad_trials_neg_z_post(end+1) = n_neg_post;
...
mean(bad_trials_neg_z)
mean(bad_trials_neg_z_post)
mean(frontalrejs)

std(bad_trials_neg_z)
std(bad_trials_neg_z_post)
std(frontalrejs)

max(bad_trials_neg_z)
max(bad_trials_neg_z_post)
max(frontalrejs)